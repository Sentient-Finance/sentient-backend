...
def get_ccip_config():
    ...
    destinations = [
        CCIPDestinationConfig(chain_name="Ethereum Sepolia", selector=CCIP_CHAIN_SELECTORS["ethereum_sepolia"]),
        CCIPDestinationConfig(chain_name="Arbitrum Sepolia", selector=CCIP_CHAIN_SELECTORS["arbitrum_sepolia"]),
        CCIPDestinationConfig(chain_name="OP Sepolia", selector=CCIP_CHAIN_SELECTORS["op_sepolia"]),
        CCIPDestinationConfig(chain_name="BNB Chain Testnet", selector=CCIP_CHAIN_SELECTORS["bnb_chain_testnet"]),
    ]
...
class EstimateFeeRequest(BaseModel):
    ...
    destination_chain_selector: int = Field(..., description="CCIP destination chain selector")
...
def estimate_ccip_fee(body: EstimateFeeRequest):
    try:
    ...
    rpc_url = _get_rpc_url(body.chain_id)
    ...
    if not ccip_router:
        raise HTTPException(status_code=400, detail=f"No CCIP router for chain_id {body.chain_id}")

    receiver_bytes = encode(["address"], [Web3.to_checksum_address(body.receiver)])
    extra_args_bytes = bytes.fromhex("97a657c9") + encode(["uint256"], [0])
    ...
    contract = w3.eth.contract(address=Web3.to_checksum_address(ccip_router), abi=router_abi)
    message = (
        receiver_bytes,
        b"",
        token_amounts,
